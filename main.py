...
def main(code,bundir=None,color='False'):
    ...
    colourful = color
    suicide = bundir
    os.makedirs(dir_path+'\\'+icao, exist_ok=True)
        ...
        if line_parts[0] == 'A' and line_parts[1] == icao:
            ...
            for runway_number in unique_runway_numbers:
                ...
                for r_line in runway_lines:
                    ...
                    if suicide != None:
                        file_path = os.path.join(dir_path+'/'+icao, f"{icao}_RW{r_number}_RECIP.xml")
                    else: 
                        file_path = os.path.join(dir_path+'/'+icao, f"{icao}_RW{r_number}.xml")
...
def gen(pattern):
    ...
    for line in codes:
        ...
        # Only airport header lines start with 'A,'
